...
#define LIB_IWMUTIL_FILENAME "lib_iwmutil2_20260810"
//////////////////////////////////////////////////////////////////////////////////////////
#include <float.h>
#include <math.h>
#include <shlwapi.h>
...
#define imain_begin() \
	...
	iConsole_EscOn(); \
	iCLI_begin()
#define imain_end() iCLI_end(EXIT_SUCCESS)
#define imain_err() iCLI_end(EXIT_FAILURE)
...
typedef struct
{
	...
	UINT uAry;	  // [配列個数] 格納されているデータの要素数
	UINT uSizeOf; // [型サイズ] 1要素あたりのバイト長 (UTF-8=1／UTF-16=2など)
	UINT uAlloc;  // [アロケート長] 実際に確保された総バイト数
...
typedef struct
{
	...
	UINT uMax;			   // 確保済みの最大数（キャパシティ） [例: 32個確保時 = 32]
	UINT uMaxId;		   // [ユニークIDカウンター] 発行済みの最大ID。1ベースで増加のみ（データ削除時も減少させない）
	UINT uEOD;			   // 次に書き込み可能な末尾インデックス [例: 32個確保時 = 0 〜 31 (32で満杯)]
} $struct_HeapManager;

$struct_HeapManager *iHM_begin(VOID);
VOID *iHM_calloc($struct_HeapManager *pMgr, UINT len, UINT sizeOf, BOOL aryOn);
...
#define irealloc_MS(ptr, n) (MS *)irealloc(ptr, (UINT)n)
#define irepalloc_MS(oldPtr, newPtr, n) (MS *)irepalloc(oldPtr, newPtr, (UINT)n)

// MS 配列
#define icalloc_MS_ary(n) (MS **)icalloc((UINT)n, sizeof(MS *), TRUE)
#define irealloc_MS_ary(ptr, n) (MS **)irealloc(ptr, (UINT)n)

// WS 文字列
#define icalloc_WS(n) (WS *)icalloc((UINT)n, sizeof(WS), FALSE)
#define irealloc_WS(ptr, n) (WS *)irealloc(ptr, (UINT)n)
#define irepalloc_WS(oldPtr, newPtr, n) (WS *)irepalloc(oldPtr, newPtr, (UINT)n)

// WS 配列
#define icalloc_WS_ary(n) (WS **)icalloc((UINT)n, sizeof(WS *), TRUE)
#define irealloc_WS_ary(ptr, n) (WS **)irealloc(ptr, (UINT)n)

// Num 配列
#define icalloc_INT(n) (INT *)icalloc((UINT)n, sizeof(INT), TRUE)
#define irealloc_INT(ptr, n) (INT *)irealloc(ptr, (UINT)n)
...
#define EP(format, ...) fprintf(stderr, format, __VA_ARGS__)
...
#define ENL() putc('\n', STDERR)
#define EP1(str) fputs(str, STDERR)
#define EP2(str) \
	EP1(str);    \
	\ ENL()
...
VOID idate_cnv_month(INT *i_y, INT *i_m, CONST INT from_m, CONST INT to_m);
// 1-12月
...
#define idate_NOW() (INT *)idate_nowToiAryYmdhns(TRUE)
